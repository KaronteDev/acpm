...
export async function usersRoutes(fastify: FastifyInstance) {
  fastify.get('/', { preHandler: authenticate }, async (request, reply) => {
    const users = await query(
      `SELECT id, email, full_name, avatar_url, role, cognitive_profile, timezone, is_active, created_at, last_seen_at
  ...
  fastify.get('/:id', { preHandler: authenticate }, async (request, reply) => {
    ...
    const user = await queryOne(
      `SELECT id, email, full_name, avatar_url, role, cognitive_profile, timezone, is_active, created_at, last_seen_at FROM users WHERE id = $1`,
      [id]
    );
